...
- 总开关（关闭 = 顶栏不装载任何状态项）
- 插件自有 6 项逐项开关：状态文本、文件系统同步、CodeBuddy、聚合器、网络位置、默认部署服务器——勾选即显示、取消即隐藏，**在任何 IDEA 版本上行为一致**
...
`View → 外观 → 状态栏微件`（或右键底部状态栏）勾选控制：菜单里勾选哪些，顶栏就显示哪些，最迟约 5 秒内生效——与底部状态栏的原生行为同源同义。
