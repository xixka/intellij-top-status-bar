...
状态项也可以在**原生「状态栏微件」菜单**中开关（右键底部状态栏，或 `View → Appearance → Status Bar Widgets`）：

- 插件自有状态项（状态文本、文件系统同步、CodeBuddy、聚合器、网络位置、默认部署服务器）注册为原生微件开关，取消勾选即从顶栏隐藏
- 镜像原生微件的项（行列号、编码、行分隔符、Git 分支等）跟随其原生微件的开关
- 两处开关为「与」关系：设置页与微件菜单都开启时才显示；微件菜单为应用级（全局），设置页为项目级
